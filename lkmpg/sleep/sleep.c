...
#include <linux/proc_fs.h> /* necessary because of procfs */
                             ...
                             loff_t *offset)
{
    ...
    /* Return 0 to signify EOF - that we have nothing more to say at this point */
    if (finished) {
        finished = 0;
        return 0;
    }
...
static int module_open(struct inode *inode, struct file *file) {
     ...
     * want to wait for the file. In this case, because the file is already
     * open, we should fail with -EAGAIN, meaning "you will have to try again",
...
static int module_close(struct inode *inode, struct file *file) {
    ...
    */
    ...
    /* Wake up all the processes in waitq, so if anybody is waiting for the file, they can have it */
    wake_up(&waitq);
