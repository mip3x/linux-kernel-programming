    ...
    static int sys_call_kprobe_pre_handler(struct kprobe *p, struct pt_regs *regs) {
        ...
        // pr_info("%s called by %d\n", syscall_sym, uid);
            ...
            const struct pt_regs *uregs = (const struct pt_regs *)regs->di;
                ...
                const char __user *upath = (const char __user *)uregs->si;
            #else
                const char __user *upath = (const char __user *)filename;
            #endif
        #else
        ...
        if (upath) {
            char buf[PATH_MAX];
            long n = strncpy_from_user(buf, upath, sizeof(buf) - 1);
            if (n < 0)
                pr_info_ratelimited("%s(uid=%u): <bad filename: %ld>\n",
                                    p->symbol_name, __kuid_val(current_uid()), n);
            else {
                buf[n] = '\0';
                pr_info_ratelimited("%s(uid=%u): %s\n",
                                    p->symbol_name, __kuid_val(current_uid()), buf);
            }
        } else
            pr_info_ratelimited("%s(uid=%u): <no filename>\n",
                                p->symbol_name, __kuid_val(current_uid()));
    ...
    */
    ...
    static unsigned long **acquire_sys_call_table(void) {
            ...
            if (sym == 0) {
                ...
                pr_info("by /boot/System.map or /proc/kallsyms, which contains all the symbol addresses,"
                    "into sym parametes.\n");
                return NULL;
            }
            sprintf_symbol(symbol, sym);
